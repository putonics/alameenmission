  ...
  React.useEffect(() => {
    ...
    if (exam.docref.id) {
      ...
      if (
        ...
        (user.regno &&
          exam.subjectGroup.subject.examType.teacherRegno === user.regno)
      ) {
        // console.log(3, 'Authentication-approved')
        setBusy(true)
        result.load(exam).finally(() => {
          if (!result.docref) {
            // console.log(4, 'Result has document')
            scrs.load(user.subscriberdocid, exam.sessionFrom).finally(() => {
              students
                .load(user.subscriberdocid, exam.sessionFrom, exam.pclass, scrs)
                .finally(() => {
                  if (students.list.length > 0) {
                    // console.log(5)
                    result.setStudents(
                      students.list.filter((s) => s.status === "ACTIVE") //! Problem solved: Only show ACTIVE
                    )
                  } else {
                    setError(
                      `No students exist in this ${exam.sessionFrom} ${exam.pclass} class!`
                    )
                  }
