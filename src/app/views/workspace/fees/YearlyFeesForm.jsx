...
/**
 * @param {{yearlyFees:Array<FeeItem>, confirm, edit: boolean, onChange: (state: any)=>void }} props 
 */
const YearlyFeesForm = props => {
    const [state, setState] = React.useState(props.yearlyFees)
    ...
    React.useEffect(() => {
        (async () => {
            if (
                ...
                props.onChange({ target: { name: 'yearlyFees', value: state } })
                if (!ok) setOk(!ok)
            } else {
                if (ok) setOk(!ok)
            }
    ...
    React.useEffect(() => {
        setHeads([new FeeItem(), ...fees.getYearlyFeesHeads().filter(f => !state.find(s => s.head === f.head))])
    }, [fees, state])
    ...
    return (
        <ContentBox>
            <StyledCard sx={ok ? { border: '2px solid #00aa44', borderBottomWidth: '6px' } : {}}>
                ...
                <Typography>Total Yearly fees: ₹{totalFees}</Typography>
            </StyledCard>
        </ContentBox>
    )
}

export default YearlyFeesForm
