...
/**
 * @param {{oneTimeFees:Array<FeeItem>, confirm, edit: boolean, onChange: (state: any)=>void }} props 
 */
const OneTimeFeesForm = props => {
    const [state, setState] = React.useState(props.oneTimeFees)
    ...
    React.useEffect(() => {
        (async () => {
            if (
                ...
                props.onChange({ target: { name: 'oneTimeFees', value: state } })
                if (!ok) setOk(!ok)
            } else {
                if (ok) setOk(!ok)
            }
    ...
    React.useEffect(() => {
        setHeads([new FeeItem(), ...fees.getOneTimeFeesHeads().filter(f => !state.find(s => s.head === f.head))])
    }, [fees, state])
    ...
    return (
        <ContentBox>
            <StyledCard sx={ok ? { border: '2px solid #00aa44', borderBottomWidth: '6px' } : {}}>
                ...
                <Typography>Total One-time fees: ₹{totalFees}</Typography>
            </StyledCard>
        </ContentBox>
    )
}
