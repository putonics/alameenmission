...
export class Institute {
  ...
  constructor(institute = null) {
    this.active = institute && institute.active ? institute.active : false
    this.branch =
      institute && institute.branch ? institute.branch : "BRANCH-NAME"
    this.district =
      institute && institute.district ? institute.district : "DISTRICT-NAME"
    this.email = institute && institute.email ? institute.email : "EMAIL-ID"
    this.institute =
      institute && institute.institute ? institute.institute : "INSTITUTE-NAME"
    ...
    this.subscriberdocid =
      institute && institute.subscriberdocid
        ? institute.subscriberdocid
        : "SUBSCRIBERDOCID"
    this.title = institute && institute.title ? institute.title : "TITLE"
    this.studentTableCols =
      institute && institute.studentTableCols
        ? institute.studentTableCols
        : null
  ...
  override = (institute = null) => {
    ...
    this.district =
      institute && institute.district ? institute.district : this.district
    this.email = institute && institute.email ? institute.email : this.email
    this.institute =
      institute && institute.institute ? institute.institute : this.institute
...
export default class Institutes {
  ...
  constructor(institutes = null) {
    ...
    this.dispatch =
      institutes && institutes.dispatch ? institutes.dispatch : null
  }

  load = async () => {
    if (this.loaded) return
    try {
      const snap = await fetchFromFirestore(
        "institutes",
        [],
      ...
      if (snap && snap.docs && snap.docs.length > 0) {
        this.list = snap.docs.map(
          (doc) => new Institute({ ...doc.data(), docref: doc.ref })
        )
        this.loaded = true
